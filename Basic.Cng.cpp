...
namespace Basic
{
    ...
    BCRYPT_KEY_HANDLE::~BCRYPT_KEY_HANDLE()
    {
        if (this->handle != 0)
        {
            ...
            if (error != 0)
                Basic::LogDebug("Basic", "BCRYPT_KEY_HANDLE::~BCRYPT_KEY_HANDLE BCryptDestroyKey failed", error);
        }
    }
    ...
    BCRYPT_ALG_HANDLE::~BCRYPT_ALG_HANDLE()
    {
        if (this->handle != 0)
        {
            ...
            if (error != 0)
                Basic::LogDebug("Basic", "BCRYPT_ALG_HANDLE::~BCRYPT_ALG_HANDLE BCryptCloseAlgorithmProvider failed", error);
        }
    }
    ...
    NCRYPT_KEY_HANDLE::~NCRYPT_KEY_HANDLE()
    {
        if (this->handle != 0)
        {
            ...
            if (error != 0)
                Basic::LogDebug("Basic", "NCRYPT_KEY_HANDLE::~NCRYPT_KEY_HANDLE NCryptFreeObject failed", error);
        }
    }
    ...
    HCERTSTORE::~HCERTSTORE()
    {
        if (this->handle != 0)
        {
            ...
            if (!success)
                Basic::LogDebug("Basic", "HCERTSTORE::~HCERTSTORE CertCloseStore failed", GetLastError());
        }
    }
    ...
    PCCERT_CONTEXT::~PCCERT_CONTEXT()
    {
        if (this->handle != 0)
        {
            ...
            if (!success)
                Basic::LogDebug("Basic", "PCCERT_CONTEXT::~PCCERT_CONTEXT CertFreeCertificateContext failed", GetLastError());
        }
    }
