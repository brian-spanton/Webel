...
namespace Basic
{
	...
	BCRYPT_KEY_HANDLE::~BCRYPT_KEY_HANDLE()
	{
		if (this->handle != 0)
		{
			...
			if (error != 0)
				Basic::globals->HandleError("BCRYPT_KEY_HANDLE::~BCRYPT_KEY_HANDLE BCryptDestroyKey", error);
		}
	}
	...
	BCRYPT_ALG_HANDLE::~BCRYPT_ALG_HANDLE()
	{
		if (this->handle != 0)
		{
			...
			if (error != 0)
				Basic::globals->HandleError("BCRYPT_ALG_HANDLE::~BCRYPT_ALG_HANDLE BCryptCloseAlgorithmProvider", error);
		}
	}
	...
	NCRYPT_KEY_HANDLE::~NCRYPT_KEY_HANDLE()
	{
		if (this->handle != 0)
		{
			...
			if (error != 0)
				Basic::globals->HandleError("NCRYPT_KEY_HANDLE::~NCRYPT_KEY_HANDLE NCryptFreeObject", error);
		}
	}
	...
	HCERTSTORE::~HCERTSTORE()
	{
		if (this->handle != 0)
		{
			...
			if (!success)
				Basic::globals->HandleError("HCERTSTORE::~HCERTSTORE CertCloseStore", GetLastError());
		}
	}
	...
	PCCERT_CONTEXT::~PCCERT_CONTEXT()
	{
		if (this->handle != 0)
		{
			...
			if (!success)
				Basic::globals->HandleError("PCCERT_CONTEXT::~PCCERT_CONTEXT CertFreeCertificateContext", GetLastError());
		}
	}
