...
namespace Web
{
	...
	bool Form::Submit(Http::Client* client, IProcess* client_completion, ByteString* client_cookie)
	{
		...
		client->Get(request, client_completion, client_cookie);
		return true;
	}
	...
	bool Form::construct_request(FormDataSet* form_data_set, Http::Request::Ref* request_result)
	{
		...
		// $ dvd.netflix.com search form seems to expect this behavior (or script is implementing it)
