...
namespace Json
{
	...
	bool Script::Execute(Html::Node* domain, Html::Node* after, Html::ElementNode::Ref* result)
	{
		if (this->element_name.item() != 0)
		{
				...
				this->parameter_value->type == Json::Value::Type::number_value)
			{
				...
				bool success = domain->find_element_with_child_count(after, element_name, (uint32)number_parameter->value, result);
				if (success)
					return true;
			}
				...
				this->parameter_value->type == Json::Value::Type::string_value)
			{
				...
				Html::ElementNode::Ref script_element;
				bool success = domain->find_element_with_attribute_prefix(after, element_name, this->attribute_name, string_parameter->value, result);
				if (success)
					return true;
			}
			else if (this->attribute_name.item() != 0 &&
				...
				this->parameter_value->type == Json::Value::Type::string_value)
			{
				...
				Html::ElementNode::Ref script_element;
				bool success = domain->find_element_with_attribute_value(after, element_name, this->attribute_name, string_parameter->value, result);
				if (success)
					return true;
			}
			else if (this->attribute_name.item() == 0 &&
				...
				this->parameter_value->type == Json::Value::Type::string_value)
			{
				...
				Html::ElementNode::Ref script_element;
				bool success = domain->find_element_with_text_value(after, element_name, string_parameter->value, result);
				if (success)
					return true;
			}
		}
	...
	bool Script::Execute(Html::Node* domain, UnicodeString::Ref* result)
	{
		...
		else
		{
				...
				this->parameter_value.item() == 0)
			{
				...
				else if (this->method_name.equals<true>(Json::globals->first_text_method))
				{
					...
					Inline<Web::TextSanitizer> stream;
	...
	void Script::deep_text(Html::Node* domain, UnicodeString::Ref* result)
	{
		UnicodeString::Ref text = New<UnicodeString>();
		text->reserve(0x100);
