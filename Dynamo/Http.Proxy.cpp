...
namespace Http
{
	...
	void Proxy::Initialize(ListenSocket* listen_socket, bool secure, Basic::Ref<IProcess> completion, ByteString::Ref cookie, Uri::Ref server_url)
	{
		__super::Initialize();
		...
		server_socket->Initialize(this, secure, &this->client_peer);
	...
	void Proxy::Process(IEvent* event, bool* yield)
	{
			...
			case Basic::EventType::ready_for_write_bytes_event:
				{
					...
					client_socket->Initialize(server_proxy, this->server_url->is_secure_scheme(), &this->server_peer);
		...
		default:
			throw new Exception("Http::Proxy::Process unexpected state");
		}
	}
