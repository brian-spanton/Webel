...
namespace Basic
{
    ...
    void HashStream::Initialize(HashAlgorithm* algorithm, byte* secret, uint32 secret_length, byte* output, uint32 output_max)
    {
        if (output_max < algorithm->hash_output_length)
            throw FatalError("Basic", "HashStream::Initialize { output_max < algorithm->hash_output_length }", 0);

        bool hmac = secret_length > 0;
        if (hmac != algorithm->hmac)
            throw FatalError("Basic", "HashStream::Initialize { hmac != algorithm->hmac }", 0);
        ...
        if (error != 0)
            throw FatalError("Basic", "HashStream::Initialize BCryptCreateHash failed", error);
    }
    ...
    void HashStream::write_elements(const byte* elements, uint32 count)
    {
        ...
        if (error != 0)
            throw FatalError("Basic", "HashStream::write_elements BCryptHashData failed", error);
    }
    ...
    void HashStream::write_eof()
    {
        ...
        if (error != 0)
            throw FatalError("Basic", "HashStream::write_eof BCryptFinishHash failed", error);
    }
}