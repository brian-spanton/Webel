...
namespace Basic
{
    ...
    bool ClientSocket::Resolve(UnicodeStringRef host, uint16 port, sockaddr_in* remoteAddress)
    {
        ...
        if (error != NO_ERROR)
        {
            Basic::LogDebug("Basic", "ClientSocket", "Resolve", "GetAddrInfoExA", WSAGetLastError());
            return false;
        }
    ...
    void ClientSocket::StartConnect(sockaddr_in remoteAddress)
    {
        if (this->state != State::new_state)
            throw FatalError("Basic", "ClientSocket", "StartConnect", "this->state != State::new_state", this->state);
        ...
        if (error == SOCKET_ERROR)
            throw FatalError("Basic", "ClientSocket", "StartConnect", "bind", WSAGetLastError());
    ...
    void ClientSocket::CompleteSend(std::shared_ptr<ByteString> bytes, uint32 count, uint32 error)
    {
        ...
        else
        {
            ...
            default:
                throw FatalError("Basic", "ClientSocket", "CompleteSend", "unhandled state", this->state);
            }
        }
    }
}