...
namespace Tls
{
    ...
    {
        void operator()(value_type* value, std::shared_ptr<IElementConsumer<byte> >* deserializer) const
        {
    ...
    template <typename value_type, int encoded_length = sizeof(value_type)>
	class NumberFrame : public IElementConsumer<byte>
    {
    private:
        uint32 received;
        value_type* value;
    ...
    template <typename vector_type>
	class VectorFrame : public StateMachine, public IElementConsumer<byte>
    {
    private:
        enum State
        {
		...
		std::shared_ptr<IElementConsumer<byte> > item_frame;
    ...
    public:
		...
		ConsumeElementsResult IElementConsumer<byte>::consume_elements(IElementSource<byte>* element_source)
        {
            ...
            case State::length_frame_pending_state:
            {
				...
				{
					this->switch_to_state(State::unexpected_length_error);
				...
				this->switch_to_state(State::next_item_state);
				return ConsumeElementsResult::in_progress;
			}
            ...
            case State::next_item_state:
            {
                ...
                {
                    this->switch_to_state(State::length_mismatch_error);
					return ConsumeElementsResult::failed;
                }
				...
				if (received < this->encoded_length)
                {
					...
					this->switch_to_state(State::item_pending_state);
					return ConsumeElementsResult::in_progress;
				}
            ...
            {
				ConsumeElementsResult result = Basic::consume_elements(this->item_frame.get(), element_source, this, State::item_frame_failed);
				if (result != ConsumeElementsResult::succeeded)
					return result;
