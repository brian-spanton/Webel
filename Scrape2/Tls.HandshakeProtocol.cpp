...
namespace Tls
{
	...
	void HandshakeProtocol::transport_received(const byte* elements, uint32 count)
	{
    ...
    void HandshakeProtocol::WriteFinished(ByteString* label)
    {
        ...
        WriteMessage(HandshakeType::finished, finished_data.get());
    }

    void HandshakeProtocol::WriteMessage(HandshakeType msg_type, IStreamWriter<byte>* frame)
    {
        Handshake handshake;
