...
namespace Basic
{
    ...
    bool ClientSocket::Resolve(UnicodeStringRef host, uint16 port, sockaddr_in* remoteAddress)
    {
        ...
        String<WCHAR> host_utf16;

		for (UnicodeString::iterator it = host->begin(); it != host->end(); it++)
		{
			host_utf16.push_back((WCHAR)*it);
		}

        // $ this DNS lookup isn't async, but should be.  possibly implement from scratch?
        int error = GetAddrInfoExW(host_utf16.c_str(), 0, NS_DNS, 0, &hints, &results, 0, 0, 0, 0);
        if (error != NO_ERROR)
        {
