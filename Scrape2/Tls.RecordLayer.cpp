...
namespace Tls
{
	...
	RecordLayer::RecordLayer(std::shared_ptr<ITransportEventHandler<byte> > application_stream, bool server, std::shared_ptr<ICertificate> certificate) :
        record_frame(&record)
    {
	...
	void RecordLayer::transport_connected()
	{
		switch (get_state())
		{
		case State::unconnected_state:
		{
			this->alert_protocol->transport_connected();
			this->heartbeat_protocol->transport_connected();
			this->handshake_protocol->transport_connected();
	...
	void RecordLayer::transport_disconnected()
	{
		DisconnectApplication();
	}

	void RecordLayer::transport_received(const byte* elements, uint32 count)
	{
		if (this->get_state() != State::record_frame_pending_state)
			throw FatalError("RecordLayer::transport_received unexpected state");

		ElementSource<byte> element_source(elements, count);

		bool success = element_source.deliver_elements(&this->record_frame);
		if (!success)
		{
			this->switch_to_state(State::record_frame_failed);
			return;
		}
    ...
    {
		std::shared_ptr<ITransportEventHandler<byte> > protocol = this->application_stream.lock();
        if (protocol.get() == 0)
            throw State::application_lost_error_1;

		this->application_connected = true;
		protocol->transport_connected();
    ...
    {
		this->alert_protocol->transport_disconnected();
		this->heartbeat_protocol->transport_disconnected();
		this->handshake_protocol->transport_disconnected();

		std::shared_ptr<ITransportEventHandler<byte> > protocol = this->application_stream.lock();
        if (protocol.get() == 0)
             return;

		protocol->transport_disconnected();
    }
        ...
        case ContentType::change_cipher_spec:
            {
				...
				this->handshake_protocol->change_cipher_spec_event();
            }
            break;
        ...
        case ContentType::alert:
            {
				...
				this->alert_protocol->transport_received(plaintext.fragment->address(), plaintext.length);
            }
            break;
        ...
        case ContentType::heartbeat_content_type:
            {
				...
				this->heartbeat_protocol->transport_received(plaintext.fragment->address(), plaintext.length);
            }
            break;
        ...
        case ContentType::handshake:
            {
				...
				this->handshake_protocol->transport_received(plaintext.fragment->address(), plaintext.length);
            }
            break;
        ...
        case ContentType::application_data:
            {
                ...
                if (plaintext.length != 0)
                {
                    protocol->transport_received(plaintext.fragment->address(), plaintext.length);
                }
            }
            break;
