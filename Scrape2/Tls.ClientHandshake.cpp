...
namespace Tls
{
	...
	void ClientHandshake::transport_connected()
	{
		...
		case State::start_state:
		{
			...
			WriteMessage(HandshakeType::client_hello, &client_hello_serializer);
	...
	void ClientHandshake::transport_disconnected()
	{
	}

	void ClientHandshake::change_cipher_spec_event()
	{
		switch (this->get_state())
		{
	...
	ConsumeElementsResult ClientHandshake::consume_elements(IElementSource<byte>* element_source)
    {
		...
		{
			ConsumeElementsResult result = Basic::consume_elements(&this->server_hello_frame, element_source, this, State::server_hello_frame_failed);
			if (result != ConsumeElementsResult::succeeded)
				return result;
		...
		{
			ConsumeElementsResult result = Basic::consume_elements(&this->certificates_frame, element_source, this, State::certificates_frame_failed);
			if (result != ConsumeElementsResult::succeeded)
				return result;
		...
		case State::expecting_server_hello_done_state:
		{
			...
			WriteMessage(HandshakeType::client_key_exchange, &encrypted_pre_master_secret_serializer);

			WriteFinished(&Tls::globals->client_finished_label);
		...
		{
			ConsumeElementsResult result = Basic::consume_elements(&this->handshake_frame, element_source, this, State::handshake_frame_4_failed);
			if (result != ConsumeElementsResult::succeeded)
				return result;
